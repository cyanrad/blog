...
func StyleHTML(inputHTML []byte) []byte {
	...
	outputHTML = bytes.ReplaceAll(outputHTML, []byte(`<code>`), []byte(`<code class="rounded-md bg-zinc-700 p-1 font-mono text-md sm:text-lg text-emerald-300">`))
	outputHTML = bytes.ReplaceAll(outputHTML, []byte(`<pre class="`), []byte(`<pre class="rounded-md min-[500px]:text-md min-[320px]:max-w-[20rem] min-[420px]:max-w-[25rem] min-[500px]:max-w-[30rem] sm:max-w-[35rem] md:max-w-[40rem] lg:max-w-[100%] `))
