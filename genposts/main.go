...
func styleHTML(inputHTML []byte) []byte {
	...
	outputHTML = bytes.ReplaceAll(outputHTML, []byte(`<em>`), []byte(`<em class="text-rose-400">`))
	...
	outputHTML = bytes.ReplaceAll(outputHTML, []byte(`<h3`), []byte(`<h3 id="a-testing-file" class="mb-4 text-2xl text-indigo-300" `))
